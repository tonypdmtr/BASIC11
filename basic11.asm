...
TBUFPTR             rmb       2                   ;token buffer pointer
          ;--------------------------------------
          ;next 5 variables must remain grouped together
          ;--------------------------------------
...
VarSize             rmb       2                   ;used by the line editor. size of the variable table
          ;--------------------------------------
                    #temp     $100-{20*3+38}      ;20 3-byte vectors + IN/OUTABLE + CONSTAT + INCONNE
          #if * > :temp
                    #Fatal    Ran out of zero-page RAM
          #else
                    #Message  Zero-page RAM remaining: {:temp-:RAM} bytes
          #endif
                    org       :temp
          ;--------------------------------------
...
DoNumExpr@@         jsr       DONEXP              ;GO DO A NUMERIC EXPRESSION
                    ...
                    lda       #' '                ;PUT A TRAILING SPACE AFTER ALL NUMBERS
...
PastComma@@         iny                           ;BUMP IP PAST THE COMMATOK
                    ...
                    lda       #' '                ;NO. GET A SPACE & PRINT TILL WE GET THERE
...
ERASEBYT            proc
                    ...
                    ldb       #$16                ;TURN PROGRAMMING VOLTAGE OFF
                    stb       PPROG,y
...
PROGBYTE            proc
                    pshy
                    ldy       IOBaseV             ;Point to the base address of the I/O Registers
Loop@@              ldb       #$02                ;SET UP NORMAL PROGRAMMING MODE, ADDRESS/DATA
                    stb       PPROG,y             ;LATCHED, PROGRAMMING VOLTAGE OFF
                    sta       ,x                  ;LATCH DATA & ADDRESS
                    psha                          ;SAVE THE DATA FOR COMPARE AFTER PROGRAMMING
                    tpa                           ;GET CURRENT I-BIT STATUS
                    psha                          ;SAVE IT
                    sei                           ;INHIBIT INTERRUPTS WHILE PROGRAMMING
                    ldb       #$03                ;TURN ON PROGRAMMING VOLTAGE
                    stb       PPROG,y
...
;*******************************************************************************
          ...
          #else
                    #Message  ROM remaining: {:temp-:ROM} bytes
          #endif
                    org       :temp
;*******************************************************************************
...
DFLOPADR            dw        $4000               ;Address of flip-flop used to connect the HC11 SCI
                    ...
                    #VECTORS  ROMBEG+ROMSIZE-42   ;START OF VECTOR TABLE (2*21)
